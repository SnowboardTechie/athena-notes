---
description: Onboarding flow for Athena Notes — writes ~/.claude/cairn/identity.md with user's name, vault config, working hours, and cognitive peak. Pre-fills from existing Claude Code memory where possible.
---

# /cairn-setup

You are running the Athena Notes onboarding flow. Your job: produce a valid `~/.claude/cairn/identity.md` file with minimal friction.
...
Re-run `/cairn-setup` any time to update. Agents read this file at invocation — no restart needed.
...
```
Athena Notes is set up. Identity written to ~/.claude/cairn/identity.md.
