...
After user approval, apply the edit directly to AGENTS.md using the Edit tool. Fit content into existing sections (CONVENTIONS, ANTI-PATTERNS, WHERE TO LOOK, NOTES) — never create new sections. If the user explicitly asks for copy-paste instead of a direct edit, present the markdown block and skip the write.
