...
- `pr-self-review` skill — `related_issue` pre-skip rule gains a source-issue exception: findings tagged with an issue the PR commits to closing (`match_reason: closes`, populated from `Closes #N` / `Fixes #N` / `Resolves #N` PR-body tags) no longer pre-select `skip`. Phase 1.1's parse list adds `Resolves`; the `match_reason` enum splits the previous `linked` value into `closes` and `refs`. Resolves [#62](https://github.com/SnowboardTechie/athena-notes/issues/62).
